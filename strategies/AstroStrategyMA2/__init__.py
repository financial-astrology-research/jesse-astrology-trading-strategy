...
class AstroStrategyMA2(Strategy):
    ...
    @property
    def stop_atr(self):
        return ta.atr(self.candles[-240:], period=self.hp['stop_atr_period'])

    @property
    def entry_atr(self):
        return ta.atr(self.candles[-240:], period=self.hp['entry_atr_period'])
    ...
    def take_profit_atr(self):
        daily_atr = ta.atr(self.daily_candles[-240:], self.hp['take_profit_atr_period'])
        return daily_atr
    ...
    def fast_ma(self):
        ...
        return ta.sma(self.candles[-240:], fast_ma_period, 'close', True)

    @property
    def slow_ma(self):
        return ta.sma(self.candles[-240:], self.hp['slow_ma_period'], 'close', True)
    ...
    def position_size(self, entry, stop):
        risk_qty = utils.risk_to_qty(self.available_margin, 10, entry, stop, self.fee_rate)
